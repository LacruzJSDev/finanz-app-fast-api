...
- `created_by` conserva quien creó la cuenta y `updated_by` quien realizó el último cambio humano, incluido archivarla o reactivarla mediante `PATCH`. Son referencias opcionales a `users` para preservar el histórico si se elimina una identidad.
