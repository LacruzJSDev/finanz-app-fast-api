...
- **`category_id`/`to_account_id` deben pertenecer al mismo grupo que la cuenta** — ambos se validan en la aplicación para devolver `409` claro y los impone PostgreSQL con triggers (`check_payment_plan_category_group` y `check_payment_plan_transfer_account_group`). Ninguna inserción directa puede saltarse el ámbito del grupo.
...
- **La recurrencia mensual/anual está anclada al calendario, no al último día recortado.** El servidor guarda internamente el día original de `next_due_date` de cada plan recurrente y lo sustituye solo cuando se cambia esa fecha explícitamente mediante `PATCH`. Así, un plan mensual creado para el 22 de septiembre genera siempre el día 22; uno anclado al 31 se ejecuta el 28/29 de febrero o el 30 de un mes corto, pero recupera el 31 en el siguiente mes que lo tenga. El mismo criterio se usa para la recurrencia anual en años sin ese día. La columna se rellena al migrar los planes preexistentes y un `CHECK` impide que se pierda en un plan recurrente.
- **Proceso diario de materialización** (fuera del ciclo petición/respuesta de la API, sin endpoint HTTP que lo dispare): un script (`app/payment_plans/run_due.py` o equivalente) se ejecuta una vez al día vía cron del sistema/contenedor — no un scheduler embebido en el proceso de FastAPI, coherente con `ARCHITECTURE.md` §9 (sin objetivo de alta disponibilidad, sin tráfico que justifique un proceso en segundo plano dentro de la API). Por cada plan activo con `next_due_date <= hoy`:
  ...
  2. Si `is_recurring`, avanza `next_due_date` sumándole `frequency_interval` unidades de `frequency_unit` **a partir de la fecha de vencimiento programada, no de la fecha real de ejecución** — para que un plan mensual del día 1 se mantenga en el día 1 aunque el cron se ejecute con retraso. Para meses/años usa el día ancla anterior: no deriva el siguiente ciclo del día 28/30 al que se haya recortado una vez. Si la nueva fecha supera `end_date`, el plan se archiva (`is_active = false`) en vez de avanzar.
  3. Si no es recurrente, el plan se archiva tras generar su única transacción.
  4. Cada plan se procesa en su propia transacción de base de datos: primero bloquea su fila con `SELECT ... FOR UPDATE` y vuelve a comprobar que sigue activo y vencido. Por ello, si dos crons recibieron el mismo listado, el segundo observa el avance del primero y no repite la operación. Además, cada ocurrencia tiene un identificador determinista de `(plan, fecha programada)` y `transactions` impone unicidad parcial por ocurrencia y cuenta: una transferencia conserva sus dos patas porque usa cuentas distintas, pero una segunda materialización completa revierte.
  5. Si falla la generación, se hace `rollback`, no se avanza ni se archiva el plan y se emite un evento de log con `payment_plan_id` y el tipo de fallo. El plan queda activo y pendiente para el siguiente cron; la alerta se configura sobre esos eventos en el recolector de logs de producción, no dentro de la lógica de negocio.
...
- Un plan mensual anclado al día 22 conserva el día 22 en todos los meses; uno anclado al 31 se recorta en febrero/meses de 30 días y recupera el día 31 en el siguiente mes que lo admite.
- Dos procesos diarios que intentan la misma ocurrencia no crean transacciones duplicadas: uno bloquea y materializa; el otro vuelve a leer el vencimiento ya avanzado y no hace nada. La restricción de ocurrencia/cuenta de `transactions` es una defensa adicional de base de datos.
